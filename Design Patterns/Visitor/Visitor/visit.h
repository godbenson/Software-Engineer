...
public:
	...
	void visitVideoCard(VideoCard * videoCard) {
		std::cout << Visitor::getName() << "is checking VideoCard circuits.(" << videoCard->getName() << ")" << std::endl;
	}

	//checking mainboard
	void visitMainBoard(MainBoard * mainboard) {
		std::cout << Visitor::getName() << "is checking Mainboard circuits.(" << mainboard->getName() << ")" << std::endl;
	}
};
